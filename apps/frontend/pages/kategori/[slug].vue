...
import type { ListingListItem, CategoryTree, PaginatedResponse, ApiResponse } from "@rafimdan/shared"
...
const currentCategory = computed(() => {
  for (const cat of categories.value) {
    if (cat.slug === categorySlug.value) return cat
...
if (!currentCategory.value) {
  throw createError({ statusCode: 404, message: "Kategori bulunamadı" })
}

const { data: listingsRes, pending } = await useFetch<ListingsResp>(
  () => {
    const p = new URLSearchParams({ category: categorySlug.value, limit: "20" })
  ...
  <div class="max-w-5xl mx-auto px-4 py-8 space-y-6">
    ...
    <template v-else>
      <div v-if="listings.length === 0" class="py-16 text-center">
        <p class="text-muted-foreground text-sm">Bu kategoride henüz ilan yok.</p>
        <NuxtLink to="/ilan-ver" class="mt-3 inline-block text-sm text-foreground underline underline-offset-2 cursor-pointer">
          İlk ilanı sen ver
        </NuxtLink>
      </div>
